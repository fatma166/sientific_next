...
import { useGetClientBooksQuery } from "@/services/api/books"
import { useLanguage } from "@/services/hooks"
import { Empty, Pagination } from "antd"
import React, { useState } from 'react'

const MainBookshelfLibraryPage = () => {
  const { t } = useLanguage()
...
const Content = ({ }) => {
  ...
  const { data, isLoading, error, refetch } = useGetClientBooksQuery({ page })
      ...
      {!data?.data?.data?.length ? (<Empty />) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3   gap-4 lg:gap-6 ">
          {data?.data?.data.map((book, k) => (
            <LibraryBookCard book={book} refetch={refetch} key={k} noCart href={book?.reader_url} />
          ))}
        </div>
      )}
...
export default MainBookshelfLibraryPage